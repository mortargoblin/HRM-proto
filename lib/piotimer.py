...
class Piotimer:
    ...
    def __init__(self, *args, mode=PERIODIC, freq=- 1, period=- 1, callback=None):
        ...
        self._sm = rp2.StateMachine(self._id, self.pio_timer, freq = 1000000)
        # set interrupt handler
        self._sm.irq(handler = self._handler, hard = True)
        # Start the StateMachine's running.
        self._sm.put(interval - 5)
        self._sm.active(1)
        
    def _handler(self, dummy):
        # use wrapper to provide timer id to callback
        # not checking for None to follow MicroPython Timer behaviour
        self._callback(self)
    ...
    def deinit(self):
        if self._sm.active():
            self._sm.active(0)
            self._available.append(self._id)
